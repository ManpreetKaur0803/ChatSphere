...
const Signup = () => {
  ...
  const postDetails = (pics) => {
    ...
    if (pics.type === "image/jpeg" || pics.type === "image/png") {
      ...
      data.append("cloud_name", "chat-sphere");
      axios
        .post("https://api.cloudinary.com/v1_1/chat-sphere/image/upload", data)
        .then((response) => {
          console.log("Cloudinary full response:", response);
          console.log("Cloudinary response data:", response.data);
          if (response.data && response.data.url) {
            setPic(response.data.url.toString());
            console.log("Uploaded image URL:", response.data.url); // Logs only the URL of the uploaded image
